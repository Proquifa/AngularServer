...
public class EmpleadoRowMapperAdmin implements RowMapper {
		...
		public Object mapRow(ResultSet rs, int arg1) throws SQLException {
			...
			nivel_Empleado = rs.getString("Nivel_General");
			if(rs.wasNull()){
				empleado.setNivelGeneral(null);
			}else{			
				empleado.setNivelGeneral(nivel_Empleado);
			}
			subproceso_Empleado = rs.getString("Subproceso");
			if(rs.wasNull()){
			...
			idSubproceso=rs.getLong("idSubproceso");
			if(rs.wasNull()){
				empleado.setIdSubproceso(null);
			}else{
				empleado.setIdSubproceso(idSubproceso);
			}
			empleado.setNombreFuncion(rs.getString("NombreFuncion"));
			empleado.setAdministrador(rs.getBoolean("Administrador"));
